...
const register = async (req: Request, res: Response): Promise<void> => {
  ...
  if (role === 'admin' && !Config.WHITELIST_ADMIN_MAIL.includes(email)) {
    ...
    logger.warn(
      `User with email ${email} attempted to register as admin but is not in the whitelist`,
    );
    return;
  }
  ...
  try {
    ...
    let tokenRecord = await saveOrUpdateToken(newUser._id, newUser.role);
    ...
    logger.error(
      `Error during user registration: ${error instanceof Error ? error.stack : error}`,
    );
  }
};
